...
let observerCheckout = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList") {
      let titlesCheckout = document.querySelectorAll(
        ".wc-block-components-checkout-step__heading .wc-block-components-title"
      );
      let paragraphCheckout = document.querySelectorAll(
        ".wc-block-components-checkout-step__container .wc-block-components-checkout-step__description"
      );
      let checkBoxText = document.querySelectorAll(
        ".wc-block-components-checkbox__label"
      );
      let orderSummary = document.querySelector(
        ".wc-block-components-order-summary__button-text"
      );
      let orderSpanBtn = document.querySelector(
        ".components-button .wc-block-components-button__text"
      );
...
let textareaObserver = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList") {
      let textareaCheckoutNotes = document.querySelector(
        'textarea[placeholder="Notes about your order."]'
      );
      if (textareaCheckoutNotes) {
        textareaCheckoutNotes.setAttribute(
          "placeholder",
          "Notas sobre tu orden"
        );
      }
    }
  });
...
let oneTimeRenderObserver = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList") {
      let returnToCartButton = document.querySelector(
        ".wc-block-components-checkout-return-to-cart-button"
      );
      let haveAnAccountSpan = document.querySelector(
      ...
      let termsAndCoditionsSpan = document.querySelector(
        ".wc-block-components-form .wc-block-checkout__terms span"
      );
      ...
      if (returnToCartButton && haveAnAccountSpan && termsAndCoditionsSpan) {
        if (returnToCartButton) {
          ...
          if (svgContent) returnToCartButton.appendChild(svgContent);
        ...
        if (termsAndCoditionsLink) {
          ...
          let newText = document.createTextNode(
            "Al proceder con tu compra aceptas nuestros Términos y condiciones y "
          );
...
let observerCart = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList") {
      let titleHeadProduct = document.querySelector(
        ".wc-block-components-sidebar-layout .wc-block-components-main table thead tr .wc-block-cart-items__header-image span"
      );
      let deleteProductElement = document.querySelectorAll(
        ".wc-block-components-sidebar-layout .wc-block-components-main table tbody tr .wc-block-cart-item__product .wc-block-cart-item__quantity .wc-block-cart-item__remove-link"
      );
      let textCartSidebar = document.querySelector(
        ".wc-block-components-sidebar-layout .wc-block-components-sidebar .wc-block-cart__payment-options .wc-block-components-express-payment-continue-rule"
      );
      let buttonCheckoutText = document.querySelector(
        ".wc-block-components-sidebar-layout .wc-block-components-sidebar .wc-block-cart__submit .wc-block-cart__submit-container a .wc-block-components-button__text"
      );
      let buttonLoader = document.querySelector(
        ".wc-block-components-sidebar-layout .wc-block-components-sidebar .wc-block-cart__submit .wc-block-cart__submit-container a .wc-block-components-spinner"
      );
      if (titleHeadProduct) {
      ...
      if (buttonLoader) {
        if (buttonCheckoutText) buttonCheckoutText.textContent = "";
      }
      if (
        titleHeadProduct &&
...
let observerCartSaveMElements = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList") {
      let saveElement1 = document.querySelectorAll(
        ".wc-block-components-sidebar-layout .wc-block-components-main .wc-block-cart-items tbody tr .wc-block-cart-item__product div .wc-block-components-product-badge"
      );
      let saveElement2 = document.querySelectorAll(
        ".wc-block-components-sidebar-layout .wc-block-components-main .wc-block-cart-items tbody tr .wc-block-cart-item__total div .wc-block-components-product-badge"
      );      
...
const privacyParagraph = document.querySelector(
...
if (privacyParagraph) {
  ...
  let newTextnode = document.createTextNode(
    "Tu información personal será utilizada para respaldar tu experiencia en este sitio web, para gestionar el acceso a tu cuenta y para otros fines descritos en nuestra "
  );
...
if (document.body.classList.contains("woocommerce-view-order")) {
  const methodOfpayElement = document.querySelector(
    ".woocommerce-view-order .woocommerce-order-details tfoot tr:nth-child(2) td"
  );
