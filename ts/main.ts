declare const ajaxEndpoints: any;
...
function removeInvalidCartValues() {
  ...
  let cartSpanThis = document.querySelector("#cart-count");
  if (
    Number(cartSpanThis?.getAttribute("value")) <= 0 ||
...
document.addEventListener("DOMContentLoaded", () => {
  document.body.addEventListener("click", (event) => {
    ...
    if (
      ...
      const quantityInput = parentElement?.querySelector<HTMLInputElement>(
      ...
      let cartValueCount = Number(cartSpanThis?.getAttribute("value") || "0");
      let currentCount =
        cartValueCount - quantity >= 0 ? cartValueCount - quantity : 0;
      cartSpanThis?.setAttribute("value", currentCount.toString());
      updateCurrentCount();
      removeInvalidCartValues();
    }
    if (
      ...
      inputElement.addEventListener("input", () => {
        ...
        inputElements.forEach((inputElement) => {
          currentCount += Number((inputElement as HTMLInputElement).value);
        });
...
shopPageBtnAddProduct.forEach((element: Element) => {
...
formAddProduct?.addEventListener("submit", async (event) => {
  event.preventDefault();
  const addToCartInputProductPage = document.querySelector<HTMLInputElement>(
    'input[name="quantity"]'
  );
  const addToCartBtnProductPage = document.querySelector(
